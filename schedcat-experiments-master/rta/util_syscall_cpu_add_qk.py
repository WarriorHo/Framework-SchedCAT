...
from consumer_qk import optimized_qk_search, calculate_delta_alpha_beta
...
def print_task_set(ts):
    ...
    for task in ts:
        ...
        print("{:<5} {:<10} {:<5} {:<10} {:<20} {:<10} {:<10}".format(
            task.id,
            task_type,
            task.partition,
            task.period,
...
def rta_nodrop_test(taskset, oh, conf, include_consumers=True):
    ...
    user_tasks = copy.deepcopy(taskset)

    # add check task set
    print_task_set(user_tasks)
    
    framework = get_framework('nodrop')
    delta, alpha, beta = framework.delta, framework.alpha, framework.beta

    qk_list = optimized_qk_search(user_tasks, int(conf.num_cpus), delta, alpha, beta)
    
    if qk_list is None:
        # add print
        print("No valid qk combination found for the task set.")
        return (0, 0)
    
    print("\nfind best consumer_period_factors (qk):", qk_list)

    consumers = []

    for t, qk in zip(user_tasks, qk_list):
        if qk is None:
            continue
        c = SporadicTask(0, qk * t.period, qk * t.period)
        c.syscall_count = t.syscall_count
        c.is_consumer = True
    ...
    # add print
    print_task_set(consumers)

    ts = TaskSystem(user_tasks + consumers)
    ts.sort(key=lambda t: t.preemption_level)
    ts.assign_ids()
    ...
    for t in user_tasks:
        higher = [h for h in ts if h.preemption_level < t.preemption_level]
        R_i = _event_residence_time_nodrop(t, higher, delta, alpha, beta)
        # print("\ntask {t.id} event residence time R_i = {:.3f}".format(t.id, R_i))
        print("\ntask {0} event residence time R_i = {1:.3f}".format(t.id, R_i))
        
    for partition in iter_partitions_ts(ts):
        part_ts = TaskSystem(partition)
        for t in part_ts:
            t.cost = framework.calculate_execution_time(t)
        if not bound_response_times_nodrop(part_ts, delta, alpha, beta):
            return (0, 0)
...
def run_cpu_num_config(conf):
    ...
    data = run_tests(confs, tests, oh)
    completed_time = time.time()
    write_util_data('output/util_syscall_cpu/cpu_num_test.txt', data, header, conf, start_time, completed_time)
