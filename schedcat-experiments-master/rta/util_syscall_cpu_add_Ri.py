...
def run_tests(confs, tests, oh):
    nodrop_fw = get_framework('nodrop')
    ...
    for conf in confs:
        ...
        for sample in range(int(conf.samples)):
            ...
            results = []
            for i, test in enumerate(tests):
                ok, _ = test[1](ts, oh, conf)
                samples[i].append(ok)
                results.append(ok)

            if results[2] == 1:
                ts_sorted = sorted(ts, key=lambda t: t.preemption_level)
                for idx, task in enumerate(ts_sorted):
                    if not getattr(task, 'is_consumer', False):
                        higher_tasks = ts_sorted[:idx]
                        R_i = _event_residence_time_nodrop(
                            task, higher_tasks,
                            delta_nd, alpha_nd, beta_nd
                        )
                        R_all.append(R_i)
        ...
        yield [conf.var] + [mean(s) for s in samples] + [res_max, res_min, res_mean]
...
def run_util_num_config(conf):
    ...
    write_util_data('output/util_syscall_cpu/util_R.txt', data, header, conf, start_time, completed_time)
...
def run_syscall_count_config(conf):
    ...
    data = run_tests(confs, tests, oh)
    completed_time = time.time()
    write_util_data('output/util_syscall_cpu/syscall_count_R.txt', data, header, conf, start_time, completed_time)
...
def run_cpu_num_config(conf):
    ...
    data = run_tests(confs, tests, oh)
    completed_time = time.time()
    write_util_data('output/util_syscall_cpu/cpu_num_R2.txt', data, header, conf, start_time, completed_time)
