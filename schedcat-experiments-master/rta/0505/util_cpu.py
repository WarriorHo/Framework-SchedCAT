...
def rta_test(taskset, oh, conf, include_consumers=False):
    ...
    per_cpu_results = []
    for cpuid in range(int(conf.num_cpus)):
        cpu_tasks = TaskSystem([t for t in ts if t.partition == cpuid])
        for t in cpu_tasks:
            t.cost = framework.calculate_execution_time(t)
        sched = int(bound_response_times(1, cpu_tasks))  # Assume bound_response_times can handle single CPU
        mem = sum(t.cost for t in cpu_tasks)  # Simple memory estimate: sum of execution costs
        per_cpu_results.append((sched, mem))
    return per_cpu_results
...
def rta_omnilog_test(taskset, oh, conf, include_consumers=False):
    ...
    for cpuid in range(int(conf.num_cpus)):
        ...
        for t in cpu_tasks:
            t.cost = framework.calculate_execution_time(t)
        sched = int(bound_response_times_omnilog(1, cpu_tasks, framework.delta))  # Single CPU for now
        mem = sum(t.cost for t in cpu_tasks)
        per_cpu_results.append((sched, mem))
    return per_cpu_results
...
def rta_nodrop_test(taskset, oh, conf, include_consumers=True):
    ...
    for cpuid in range(int(conf.num_cpus)):
        ...
        for t in cpu_tasks:
            t.cost = framework.calculate_execution_time(t)
        sched = int(bound_response_times_nodrop(cpu_tasks, framework.delta, framework.alpha, framework.beta))
        mem = sum(t.cost for t in cpu_tasks)
        per_cpu_results.append((sched, mem))
    return per_cpu_results
...
def run_tests(confs, tests, oh):
    for conf in confs:
        samples = [[[] for _ in range(int(conf.num_cpus))] for _ in tests]
        for sample in range(int(conf.samples)):
            ...
            for i, test in enumerate(tests):
                per_cpu_results = test[1](ts, oh, conf)
                for cpuid, (sched, mem) in enumerate(per_cpu_results):
                    samples[i][cpuid].append((sched, mem))
        row = [conf.var]
        for test_samples in samples:
            for cpuid, cpu_samples in enumerate(test_samples):
                sched_mean = mean([s for s, m in cpu_samples])
                mem_mean = mean([m for s, m in cpu_samples])
                row.extend([sched_mean, mem_mean])
        yield row
...
def run_util_num_config(conf):
    ...
    tests = setup_tests()
    header = ['UTILIZATION'] + ["{}_CPU{}_{}".format(title, cpuid, metric) 
                            for title, _ in tests 
                            for cpuid in range(int(confs[0].num_cpus)) 
                            for metric in ['SCHED', 'MEM']]
...
if __name__ == "__main__":
    class Conf:
        def __init__(self):
            ...
            self.output = 'output/util_cpu.txt'
            self.num_task = 10
            self.samples = 5000
            self.periods = '10-100'
            self.num_cpus = 2  # Example: testing with 2 CPUs
